...
var require_traverse = __commonJS({
  "../../node_modules/@babel/types/lib/traverse/traverse.js"(exports2) {
    ...
    exports2.default = traverse6;
    var _index = require_definitions();
    function traverse6(node, handlers, state) {
...
var require_whitespace = __commonJS({
  "../../node_modules/@babel/generator/lib/node/whitespace.js"(exports2) {
    ...
    function isType(node) {
      return isLiteral(node) || isObjectExpression3(node) || isArrayExpression3(node) || isIdentifier5(node) || isMemberExpression3(node);
    }
...
var require_parentheses = __commonJS({
  "../../node_modules/@babel/generator/lib/node/parentheses.js"(exports2) {
    ...
    function OptionalMemberExpression(node, parent) {
      return isCallExpression4(parent) && parent.callee === node || isMemberExpression3(parent) && parent.object === node;
    }
  ...
  "../../node_modules/@babel/generator/lib/generators/expressions.js"(exports2) {
    ...
    function NewExpression(node, parent) {
      ...
      if (this.format.minified && node.arguments.length === 0 && !node.optional && !isCallExpression4(parent, {
        callee: node
      }) && !isMemberExpression3(parent) && !isNewExpression(parent)) {
        return;
      }
...
__export(index_exports, {
  ...
  injectDomainPlugin: () => vite_plugin_inject_domain_default,
  injectOnErrorPlugin: () => vite_plugin_inject_onerror_default,
  injectedGuiListenerPlugin: () => injectedGuiListenerPlugin,
  locationRoutePlugin: () => vite_plugin_location_route_default,
...
// src/plugins/vite-plugin-location-route.ts
var import_magic_string = __toESM(require("magic-string"));
var import_parser = require("@babel/parser");
var import_traverse = __toESM(require("@babel/traverse"));
var traverseFn = typeof import_traverse.default === "function" ? import_traverse.default : import_traverse.default.default;
function replaceLocationPlugin() {
  const getMemberExpressionPath = (node) => {
    if (node.type === "MemberExpression") {
      return getMemberExpressionPath(node.object) + "." + getMemberExpressionPath(node.property);
    }
    return node.name;
  };
  const isLeftNodeLocationHref = (node) => {
    if (node.type === "MemberExpression") {
      const path8 = getMemberExpressionPath(node);
      return ["window.location.href", "location.href"].includes(path8);
    }
    return false;
  };
  ...
  return {
    name: "vite-plugin-location-replace",
    enforce: "pre",
    transform(code, id) {
      if (id.includes("node_modules") || ![".ts", ".tsx", ".js"].some((item) => id.endsWith(item))) {
        return;
      }
      ...
      try {
        ...
        traverseFn(ast, {
          AssignmentExpression(node) {
            ...
            if (nodeDetail?.left && isLeftNodeLocationHref(nodeDetail.left) && nodeDetail.operator === "=") {
              const rightNodeRange = nodeDetail.right ? getNodeRange(nodeDetail.right) : [0, 0];
              const selfNodeRange = getNodeRange(nodeDetail);
              const rightStr = code.slice(rightNodeRange[0], rightNodeRange[1]);
              const updateNavigateStr = `(function(){window.history.pushState({}, '', ${rightStr}); window.dispatchEvent(new Event('popstate'))})()`;
              selfNodeRange[0] + selfNodeRange[1] && magicString.overwrite(
                selfNodeRange[0],
                selfNodeRange[1],
                updateNavigateStr
              );
          ...
          CallExpression(node) {
            const nodeDetail = node.node;
            if (nodeDetail?.callee && [
              "location.assign",
              "location.replace",
              "window.location.assign",
              "window.location.replace"
            ].includes(getMemberExpressionPath(nodeDetail.callee))) {
              const args = nodeDetail?.arguments;
              if (args?.length) {
                const arg = args[0];
                const argRange = getNodeRange(arg);
                if (argRange[0] + argRange[1] !== 0) {
                  const argStr = code.slice(argRange[0], argRange[1]);
                  const updateNavigateStr = `window.history.pushState({}, '', ${argStr}); window.dispatchEvent(new Event('popstate'));`;
                  const selfNodeRange = getNodeRange(nodeDetail);
                  selfNodeRange[0] + selfNodeRange[1] && magicString.overwrite(
                    selfNodeRange[0],
                    selfNodeRange[1],
        ...
        console.log(
          `\u3010replaceLocationPlugin\u3011\u7F16\u8BD1\u9047\u5230\u9519\u8BEF\uFF0C\u6587\u4EF6\u8DEF\u5F84\uFF1A${id}\uFF0C\u9519\u8BEF\u4FE1\u606F\uFF1A`,
          error
        );
      }
      return null;
...
var vite_plugin_location_route_default = replaceLocationPlugin;
...
var resolveReference = async (binding, option, currentFilePath, context) => {
  if (!binding) return;
      ...
      for (const property of element.properties) {
        if (t.isObjectProperty(property)) {
          ...
          if (!key || !["path", "name", "title", "element", "children"].includes(key))
            continue;
          if (t.isJSXElement(property.value) && key === "element") {
            ...
            let currentPath = obj.path ? joinPaths(parentPath, obj.path) : parentPath;
...
var resolveImportedValue = async (binding, currentFilePath, option, context) => {
  ...
  if (binding.kind === "module") {
    ...
    try {
      ...
      traverse2(ast, {
        ...
        VariableDeclarator(path8) {
          if (t.isIdentifier(path8.node.id) && path8.node.init && t.isCallExpression(path8.node.init) && t.isIdentifier(path8.node.init.callee) && path8.node.init.callee.name === "lazy") {
    ...
    return await resolveReference(binding, option, currentFilePath, context);
  }
};
...
var collectDeps = (mod, seen = /* @__PURE__ */ new Set()) => {
  if (!mod || seen.has(mod.id) || mod.id.includes("node_modules")) return [];
  seen.add(mod.id);
  let deps = [mod.id];
  mod.importedModules.forEach((m) => {
    deps = deps.concat(collectDeps(m, seen));
  });
  return deps;
};

// src/plugins/vite-plugin-extract-router/route.ts
var traverse3 = resolveBabelModule(import_traverse3.default);
var collectRouteNodeAndPath = (code, id) => {
  try {
    ...
    traverse3(ast, {
      ...
      // 收集懒加载
      VariableDeclarator(path8) {
        if (t2.isIdentifier(path8.node.id) && path8.node.init && t2.isCallExpression(path8.node.init) && t2.isIdentifier(path8.node.init.callee) && path8.node.init.callee.name === "lazy") {
          const lazyCall = path8.node.init;
...
var extractRouterPlugin = () => {
  ...
  return {
    ...
    async transform(code, id) {
      if (id.includes("node_modules") || ![".ts", "tsx", "jsx", "js"].some((suffix) => id.endsWith(suffix)))
        return;
      ...
      const result = useRoutes.concat(normalRoutes);
      routes = new Map(result.map((item) => [item.path, item]));
    },
...
var traverse4 = resolveBabelModule2(import_traverse4.default);
...
// src/plugins/vite-plugin-inject-domain.ts
function injectDomainPlugin(domain) {
  return {
    name: "vite-plugin-inject-domain",
    enforce: "pre",
    transformIndexHtml(html) {
      return {
        html,
        tags: [
          {
            ...
            children: `
              try {
                ...
                console.log('document.domain set to ${domain}');
              } catch(e) {
...
var vite_plugin_inject_domain_default = injectDomainPlugin;

// src/plugins/vite-plugin-gui-listener.ts
var CSS = `
[data-editor-hover='true'] {
  outline: #2468f2 solid 1px !important;
  outline-offset: -1px !important;
}

[data-editor-each='true'] {
  outline: #2468f2 dashed 1px !important;
  outline-offset: -1px !important;
}

[data-editor-active='true'] {
  outline: #2468f2 solid 2px !important;
...
var OUTER_COMPONENT_LIB = [LUCIDE_REACT, CHART_REACT];
    ...
    (acc, attr) => {
      if (attr.type === "JSXAttribute") {
        ...
        if (attr.value?.type === "StringLiteral") {
          ...
          if (isAsset) {
            acc.attributes[attr.name.name] = `<<${attr.value.expression.name}>>`;
          }
        }
      }
      return acc;
...
var tagger = async (code, id, subPath, config, viteConfig) => {
  try {
    ...
    const magicString = new import_magic_string2.default(code);
    if (config.isAtomComponentPath(subPath)) {
      ...
      const exports2 = defaultExport ? namedExports.concat([defaultExport]) : namedExports;
      for (const _export of exports2) {
        ...
        if (returnStatement.argument?.type === "JSXElement") {
          ...
          openingElement.name.end && magicString.appendLeft(
      ...
      walk2(ast.program, {
        ...
        enter(node) {
          ...
          if (isSvgElement(node, importTagMap)) {
            ...
            const legacyIds = `data-component-is-root="${enterJsxElementCount === 1}" data-component-path="${subPath}" data-component-line="${line}" data-component-file="${fileName}" data-component-name="${elementName}" data-component-content="${encodeURIComponent(
          ...
          if (node.type === "JSXOpeningElement") {
            ...
            if (["Fragment", "React.Fragment"].includes(elementName)) return;
            ...
            if (shouldTag) {
              let legacyIds = `data-component-is-root="${enterJsxElementCount === 1}" data-component-path="${subPath}" data-component-line="${line}" data-component-file="${fileName}" data-component-name="${elementName}"`;
              const encodedContents = JSON.stringify(content);
              if (/<<([^>]+)>>/.test(encodedContents)) {
                legacyIds += " data-component-content={encodeURIComponent(`" + encodedContents.replace(/<<([^>]+)>>/g, (_, identifier) => {
                  return "${ " + identifier + " }";
                }) + "`)}";
              } else {
...
var defaultOptions = {
  ...
  shouldTagElement: (tagName) => {
    return !THREE_FIBER_ELEMENTS.includes(tagName) && !UNSUPPORTED_ELEMENTS.includes(tagName);
  }
};
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (id.includes(import_path6.default.normalize("/app.config.ts"))) {
        try {
          const ast = (0, import_parser6.parse)(code, {
            sourceType: "module"
          });
...
function parseSupabaseCode(content, hasSonner) {
  ...
  const ast = recast.parse(content, {
    parser: {
      parse(source) {
        return (0, import_parser7.parse)(source, {
          sourceType: "module",
